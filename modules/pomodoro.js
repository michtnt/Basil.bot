...
const startPomodoro = (name = "", id) => {
  ...
  if(breakStart){ // continuing pomodoro session
    breakStart = false;
    sendMessenger(id, { "text" : `Your break is done. Back to work!`})
  } else { // new pomodoro session
    sendMessenger(id, { "text" : `Starting Pomodoro Session ${name} on ${new Date().toLocaleString()}` })
  }
}
...
const startBreak = (id) => {
  ...
  let breakTime = 5;
  if(pomodoroCount % 4 == 0){ breakTime = 30 }
  timer.push(setTimeout(() => startPomodoro(sessionName, id), breakTime * 60000))
  sendMessenger(id, { "text" : `Pomodoro session ${pomodoroCount}. ${breakTime} minute(s) break. Enjoy!` })
}

const getStatus = () => {
  let diffTime = new Date().getTime() - startTime.getTime();
  if(pomodoroStart){ // if there is pomodoro session; 25 minutes podomore per session
  ...
  } else if(breakStart){ // every 4th pomodoro: 30 mins break; else: 5 mins
    let breakMin = 5;
    if(pomodoroCount % 4 == 0){ breakMin = 30 }
